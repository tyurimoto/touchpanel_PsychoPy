...
state "操作" as group.Operation {
    ...
    state "試行" as group.Trial {
        ...
        state "トリガ画面タッチ待ち" as state.WaitingForTouchTrigerScreen {
            ...
            state.WaitingForTouchTrigerScreen : * 天井ランプをOFF
            ...
            state.WaitingForTouchTrigerScreen --> state.Training2Delay : 画面タッチ\n(訓練)
            ...
            state.WaitingForTouchTrigerScreen : 
            state.WaitingForTouchTrigerScreen --> state.DelayMatch2Delay : 画面タッチ\n(ディレイマッチ)
            ...
            state.WaitingForTouchTrigerScreen --> state.IlegalExitDetection : イリーガル\n退室検知
        }
            state "ランダムディレイ" as state.DelayMatch2Delay{
                state.DelayMatch2Delay : do / タイムアウト監視処理
                state.DelayMatch2Delay :
                state.DelayMatch2Delay --> state.DelayMatchImage : タイムアウト\n(正常系)
                state.DelayMatch2Delay : exit / タイムアウト処理(正常系)
                state.DelayMatch2Delay : * ストップウォッチをリスタート
                state.DelayMatch2Delay : * 指定時間ランダムタイムディレイ
                state.DelayMatch2Delay --> state.IlegalExitDetection : イリーガル\n退室検知
            }
            state "ランダムディレイ" as state.Training2Delay{
                state.Training2Delay : do / タイムアウト監視処理
                state.Training2Delay :
                state.Training2Delay --> state.Training : タイムアウト\n(正常系)
                state.Training2Delay : exit / タイムアウト処理(正常系)
                state.Training2Delay : * ストップウォッチをリスタート
                state.Training2Delay : * 指定時間ランダムタイムディレイ
                state.Training2Delay --> state.IlegalExitDetection : イリーガル\n退室検知
            }
        state "ディレイマッチ" as group.DelayMatch {
            state "ディレイマッチ(正解図形表示)" as state.DelayMatchImage {
                ...
                state.DelayMatchImage --> state.IlegalExitDetection : イリーガル\n退室検知
            }
            state "ディレイマッチ(タッチパネル画面を黒)" as state.DelayMatch2Black {
                ...
                state.DelayMatch2Black --> state.IlegalExitDetection : イリーガル\n退室検知
            }
            ...
            state "訓練" as state.Training {
                ...
                state.Training : * タッチコレクトをストップ
                state.Training --> state.IlegalExitDetection : イリーガル\n退室検知
            }
        }
        ...
        state "正解" as group.CorrectAnswer {
            state "正解" as state.CorrectAnswer {
                ...
                state.CorrectAnswer --> state.DelayFeed
                state.CorrectAnswer : exit / 処理
                state.CorrectAnswer : * タッチパネル画面を黒
                state.CorrectAnswer : * 天井ランプをOFF
                'state.CorrectAnswer : * フィードランプをON
                state.CorrectAnswer : * 正解音を再生
                'state.CorrectAnswer : * 給餌を実行
            }
            state "給餌待ち" as state.DelayFeed{
                state.DelayFeed --> state.WaitingForReward : タイムアウト(正常系)
                state.DelayFeed : do / タイムアウト監視処理
                state.DelayFeed : exit / タイムアウト処理(正常系)
                state.DelayFeed : * フィードランプをON
                state.DelayFeed : * 報酬音を再生
                state.DelayFeed : * ストップウォッチをリスタート
            }
            state "給餌完了" as state.WaitingForReward{
                state.WaitingForReward --> state.WaitingDelayForRoomLump : タイムアウト(正常系)
                state.WaitingForReward : do / タイムアウト監視処理
                state.WaitingForReward : exit / タイムアウト処理(正常系)
                state.WaitingForReward : * 給餌を実行
                state.WaitingForReward : * ストップウォッチをリスタート
            }
            state "ルームランプ点灯ディレイ" as state.WaitingDelayForRoomLump{
                state.WaitingDelayForRoomLump --> state.TurnOnRoomLump : タイムアウト(正常系)
                state.WaitingDelayForRoomLump : do / タイムアウト監視処理
                state.WaitingDelayForRoomLump : exit / タイムアウト処理(正常系)
                state.WaitingDelayForRoomLump : * フィードランプをOFF
                state.WaitingDelayForRoomLump : * ストップウォッチをリスタート
            }
            state "ルームランプ点灯" as state.TurnOnRoomLump{
                state.TurnOnRoomLump --> state.WaitingForFeedComplete : タイムアウト(正常系)
                state.TurnOnRoomLump : do / タイムアウト監視処理
                state.TurnOnRoomLump : exit / タイムアウト処理(正常系)
                state.TurnOnRoomLump : * タッチパネル画面を黒
                state.TurnOnRoomLump : * 天井ランプをON
            }
            ...
            state "給餌完了待ち" as state.WaitingForFeedComplete {
                ...
                state.WaitingForFeedComplete --> state.IlegalExitDetection : イリーガル\n退室検知
            }
        }
