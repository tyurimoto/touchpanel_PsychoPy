...
int detect_raw_image(char* data, int w, int h, int c, bbox_t_container &container, double thresh = 0.2)
{
    ...
    memcpy(img.data, im.data, im.w*im.h*im.c * sizeof(float));

    std::vector<bbox_t> detection = detector->detect(img, (float)thresh);
    for (size_t i = 0; i < detection.size() && i < C_SHARP_MAX_OBJECTS; ++i)
        container.candidates[i] = detection[i];
