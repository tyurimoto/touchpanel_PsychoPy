...
{
    public partial class WMTouchPictureBox : PictureBox
    {
        public WMTouchPictureBox()
        {
            InitializeComponent();
            HandleCreated += new System.EventHandler(this.OnLoadHandler);
            touchInputSize = Marshal.SizeOf(new TOUCHINPUT());
            touch = new TouchInputStore();
            Microsoft.Win32.RegistryKey registryKey = Microsoft.Win32.Registry.CurrentUser.OpenSubKey(@"Control Panel\Cursors", true);
            if(registryKey!=null)
            {
                registryKey.SetValue("ContactVisualization", 0);
                registryKey.SetValue("GestureVisualization", 0);
            }
        ...
        private TouchInputStore touch;

        // Touch event handlers
        [Browsable(true)]
        [DesignerSerializationVisibility(DesignerSerializationVisibility.Visible)]
        [Category("アクション")]
        [Description("1点のみタップされたとき発生")]
        public event EventHandler<MouseEventArgs> TappedOnlyOnePoint;
        protected virtual void OnTappedOnlyOnePint(MouseEventArgs e)
        {
            EventHandler<MouseEventArgs> handler = TappedOnlyOnePoint;
        ...
        private void OnTouchDown(Object sender, WMTouchEventArgs e)
        {
            if(e.IsPrimaryContact)
            {
                ...
                touch.Flags = e.Flags;
                touch.Mask = e.Mask;
                Debug.WriteLine("ID: {0} Time: {1} Primary: {2} TouchDown", touch.Id, e.Time - touch.Time, e.IsPrimaryContact);
                Debug.WriteLine("X: {0}, Y: {1}", e.LocationX, e.LocationY);
            }
            else
            {
                Debug.WriteLine("ID: {0} Time: {1} Primary: {2} TouchDown", touch.Id, e.Time - touch.Time, e.IsPrimaryContact);
        ...
        private void OnTouchUp(Object sender, WMTouchEventArgs e)
        {
            if(e.IsPrimaryContact && e.Id == touch.Id)
            {
                Debug.WriteLine("ID: {0} Time: {1} Primary: {2} TouchUp", touch.Id, e.Time - touch.Time, e.IsPrimaryContact);
                Debug.WriteLine("X: {0}, Y: {1}", e.LocationX, e.LocationY);
                MouseEventArgs eventArgs = new MouseEventArgs(MouseButtons.Left, 1, e.LocationX, e.LocationY, 0);
                OnTappedOnlyOnePint(eventArgs);
            }
        }
        ...
        class TouchInputStore
        {
            ...
            private int id = 0;                 // contact ID
            private int mask = 0;               // mask which fields in the structure are valid
            private int flags = 0;              // flags
            private int time = 0;               // touch event time
